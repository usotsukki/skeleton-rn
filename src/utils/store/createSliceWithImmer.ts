...
type SliceActions<Value> = {
	[actionName: string]: (...args: never[]) => (draft: Draft<Value>) => Draft<Value> | void
}

export type SliceConfig<Name extends string, Value, Actions extends SliceActions<Value>> = {
	name: Name
	value: Value
	actions: Actions
}
...
type ImmerActions<Value, Actions extends SliceActions<Value>> = {
	[K in keyof Actions]: (...args: InferArgs<Actions[K]>) => (prev: Value) => Value
}

export function createSliceWithImmer<Name extends string, Value, Actions extends SliceActions<Value>>(
