...
describe('ImageWithBoundingBoxes component', () => {
	...
	it('renders an image with localUri', () => {
		...
		const { getByTestId } = render(<ImageWithBoundingBoxes image={{ localUri }} testId={testId} />)
	...
	it('renders an image with uri when localUri is not provided', () => {
		...
		const { getByTestId } = render(<ImageWithBoundingBoxes image={{ uri }} testId={testId} />)
	...
	it('renders bounding boxes correctly', () => {
		...
		const { getByTestId } = render(<ImageWithBoundingBoxes boundingBoxes={boundingBoxes} testId="test" />)
		expect(getByTestId('test-boundingBoxView--boundingBox--x10y10-w100h50')).toBeTruthy()
	})
	...
	it('applies style to the outer view', () => {
		...
		const { getByTestId } = render(<ImageWithBoundingBoxes style={style} testId="test" />)
	...
	it('applies imageStyle to the image', () => {
		...
		const { getByTestId } = render(<ImageWithBoundingBoxes imageStyle={imageStyle} testId="test" />)
		expect(getByTestId('test-image').props.style).toContainEqual(imageStyle)
	})

	it('adjusts image size based on contentFit prop', () => {
		const { getByTestId, rerender } = render(<ImageWithBoundingBoxes contentFit="fill" testId="test" />)
