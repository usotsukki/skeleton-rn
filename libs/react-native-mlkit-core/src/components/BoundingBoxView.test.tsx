...
import { BoundingBox, useBoundingBoxStyle } from '../hooks'
...
describe('BoundingBoxView component', () => {
	it('renders correctly without a label', () => {
		...
		const { queryByTestId } = render(
			<BoundingBoxView box={mockBox} scale={{ x: 1, y: 1, offsetX: 0, offsetY: 0 }} testId="test" />,
		)
	...
	it('renders correctly with a label', () => {
		...
		const { getByTestId } = render(
			<BoundingBoxView box={labelBox} scale={{ x: 1, y: 1, offsetX: 0, offsetY: 0 }} testId="test" />,
		)
	...
	it('renders with different scales', () => {
		...
		const { getByTestId } = render(
			<BoundingBoxView box={labelBox} scale={{ x: 2, y: 2, offsetX: 0, offsetY: 0 }} testId="test" />,
		)
