...
export function useImageScale(
	...
	useEffect(() => {
		...
		const containScale = Math.min(container.width / image.width, container.height / image.height)
		const scaleDownScale = Math.min(1, Math.min(container.width / image.width, container.height / image.height))
		...
		setScale(newScale)
	}, [contentFit, container.width, container.height, image?.width, image?.height])
